...
class FEmodule(nn.Module):
    def __init__(self,in_channels,out_channels):
        ...
        self.dilated_liner = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=1, kernel_size=2, stride=1, dilation=6, padding=0),
            nn.ReLU(inplace=True),
        ...
        self.stage = nn.ModuleList([
            Stagemodule(in_channels,out_channels) for in_channels,out_channels in zip([64,128,256,384,512],out_channels_list[-5:])
        ])
    ...
    def forward(self,x_1,x_8):
        outputs_stage = []
        #x_p = x
        x_1 = self.dilated_liner(x_1)
        outputs_stage.append(x_1)
        ...
        temp = self.conv[4](x_8)
        x3 = x3*temp
        x3 = self.conv[2](x3)

        temp = self.conv[5](x_8)
        x4 = x4 * temp
        x4 = self.conv[3](x4)

        temp = self.conv[6](x_8)
        x5 = x5 * temp

        temp = self.conv[7](x_8)
        x6 = x6*temp
...
class SCRBnetV7(nn.Module):
    ...
    def forward(self, x_1,x_8):
        ...
        x1[masks] = torch.add(x1[masks],x_8[masks])*0.5
        return x1
...
if __name__ == '__main__':
    ...
    x_1 = torch.randn(1, in_channels, height, width)
    x_8 = torch.randn(1, 8, height, width)
    #fe = FEmodule(in_channels, 64)
    #print(fe)
    net = SCRBnetV7(8, out_channels=8,masks=masks_tensor)
    y = net(x_1,x_8)
    print(y.shape)


    input = torch.randn(1, 8, 480, 636)
    flops, params = profile(net, inputs=(x_1,x_8))
