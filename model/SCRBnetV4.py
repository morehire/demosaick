...
class ECAblock(nn.Module):
    def __init__(self,in_channels,out_channels,serial,kernel_size=3,padding=1,stride=1):
        super(ECAblock, self).__init__()
        self.serial = serial
        if serial>0:
            # 在之后不改变大小和通道的卷积中使用
            self.conv_dd = nn.Conv2d(in_channels,out_channels//2,1)
        ...
        if serial>0:
            # 在之后不改变大小和通道的卷积中使用
            self.conv_ad = nn.Conv2d(in_channels, out_channels*2, 1)
            in_channels = in_channels*2
            out_channels = out_channels*2
...
class Stagemodule(nn.Module):
    def __init__(self,in_channels,out_channels,kernel_size,padding):
        ...
        self.eca_conv = nn.ModuleList([
            ECAblock(in_channels, out_channels, kernel_size=kernel_size,padding=padding,stride=stride,serial=i)
            for in_channels,stride,i in zip([in_channels,out_channels,out_channels],[2,1,1],[0,1,2])
        ])
...
if __name__ == '__main__':
    ...
    #print(fe)
    net = SCRBnetV4(in_channels, out_channels=8,masks=masks_tensor,scale=1.001)
    #
